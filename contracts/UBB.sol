...
contract UBB is SIC {
    ...
    constructor() SIC("Universitatea Babes Bolyai", "UBB", "Mihai Kogalniceanul 5."){
    }
    ...
    function addFaculty(
        ...
        require(_faculties[symbol].participationDate==0, 'This Faculty is already added to the contract.');
        _faculties[symbol].facultyName = name;
        _faculties[symbol].facultySymbol = symbol;
        _faculties[symbol].participationDate = block.timestamp;
    }
    ...
    function addSpecializationWithFaculty(
        ...
        require(_specializations[specName].available==false, "This specialization is already added to the contract");
        require(_faculties[faculty].participationDate>0, "Unknown Faculty");
    ...
    function getSpecializationWithFaculty(
        string memory specName
    ) external view returns (Specialization memory, string memory) {
